...
public class CountInSorted {
    ...
    public static int countSorted(int[] sortedA, int element) {
        ...
        int last = findLast(sortedA, element, first, sortedA.length - 1);

        return last - first + 1;
    }

    private static int findFirst(int[] a, int element, int low, int high) {
        if (low > high) {
            return -1;
        }
        ...
        comparisons++;
        if (a[mid] == element) {
            int left = findFirst(a, element, low, mid - 1);
            return (left == -1) ? mid : left;
        } else if (a[mid] > element) {
            return findFirst(a, element, low, mid - 1);
        } else {
            return findFirst(a, element, mid + 1, high);
        }
    }

    private static int findLast(int[] a, int element, int low, int high) {
        if (low > high) {
            return -1;
        }
        ...
        if (a[mid] == element) {
            int right = findLast(a, element, mid + 1, high);
            return (right == -1) ? mid : right;
        } else if (a[mid] > element) {
            return findLast(a, element, low, mid - 1);
        } else {
            return findLast(a, element, mid + 1, high);
        }
    }
