...
public class CountInSorted {
	...
	public static int countSorted (int [] sortedA, int element) {
		return countTargetRecursive(sortedA, element, 0, sortedA.length - 1);	
	}

    private static int countTargetRecursive(int[] arr, int target, int low, int high) {
        if (low > high)
            return 0;
        ...
        if (arr[mid] == target)
            return 1 + countTargetRecursive(arr, target, low, mid - 1) + countTargetRecursive(arr, target, mid + 1, high);
        
        else if (arr[mid] < target)
            return countTargetRecursive(arr, target, mid + 1, high);
        
        return countTargetRecursive(arr, target, low, mid - 1);
    }
