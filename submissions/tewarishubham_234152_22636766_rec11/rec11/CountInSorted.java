...
public class CountInSorted {
    ...
    private static int findLast(int[] sortedA, int element, int low, int high) {
        if (low > high) {
            return -1;
        }
        ...
        if (sortedA[mid] == element) {
            if (mid == high || sortedA[mid + 1] != element) {
                return mid;
            } else {
                return findLast(sortedA, element, mid + 1, high);
            }
        } else if (sortedA[mid] < element) {
            return findLast(sortedA, element, mid + 1, high);
        } else {
            return findLast(sortedA, element, low, mid - 1);
        }
    }
    
    private static int findFirst(int[] sortedA, int element, int low, int high) {
        if (low > high) {
            return -1;  
        }
        int mid = low + (high - low) / 2;
        ...
        if (sortedA[mid] == element) {
            if (mid == 0 || sortedA[mid - 1] != element) {
                ...
                return findFirst(sortedA, element, low, mid - 1);
            }
        } else if (sortedA[mid] < element) {
            return findFirst(sortedA, element, mid + 1, high);
        } else {
            return findFirst(sortedA, element, low, mid - 1);
        }
    }
