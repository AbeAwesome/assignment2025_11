...
public class CountInSorted {
    ...
    public static int countSorted (int [] sortedA, int element) {
        ...
        int left = findFirst(sortedA, element, 0, sortedA.length - 1);
        if (left == -1) {
            return 0;
        }
        int right = findLast(sortedA, element, 0, sortedA.length - 1);
        return right - left + 1;
    }
    ...
    private static int findFirst(int[] a, int x, int low, int high) {
        ...
        comparisons++;
        if (a[mid] == x) {
            int left = findFirst(a, x, low, mid - 1);
            if (left == -1) {
                return mid;
            }
            return left;
    ...
    private static int findLast(int[] a, int x, int low, int high) {
        ...
        comparisons++;
        if (a[mid] == x) {
            int right = findLast(a, x, mid + 1, high);
            if (right == -1) {
                return mid;
            }
            return right;
