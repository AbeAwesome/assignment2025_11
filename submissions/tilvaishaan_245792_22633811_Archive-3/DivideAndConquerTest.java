...
class DivideAndConquerTest {
	...
	void testCountSorted10() {		
		...
		for(int i = 0; i < a.length; ) {
			...
			Assertions.assertEquals(currCount, CountInSorted.countSorted(a, curr));
		}		
	}
	...
	void testCountSorted1024() {		
		...
		Assertions.assertEquals(12, CountInSorted.countSorted(a, element));
		
		// now see if the complexity is log n
		Assertions.assertTrue(CountInSorted.comparisons <= 21);
	}
	...
	void testCountSorted512Repeats() {		
		...
		// assert that the iterative calculated count is equal to that of the recursive
		Assertions.assertEquals(512, CountInSorted.countSorted(a, element));
		
		// now see if the complexity is log n
		Assertions.assertTrue(CountInSorted.comparisons <= 19);
	}
	...
	void testCountSorted1024Repeats() {		
		...
		// assert that the iterative calculated count is equal to that of the recursive
		Assertions.assertEquals(1024, CountInSorted.countSorted(a, element));
	...
	void testDirtyArray() {		
		...
		int actualPos = FindDirty.findInDirtyArray(a, element);
		Assertions.assertTrue(actualPos >= minElementIndex && actualPos <= maxElementIndex);
		...
		Assertions.assertTrue((double)FindDirty.comparisons <= maxComparisons);
	}
