...
public class RecursionChallenge {	
	...
	public static String allStar(String astring) {
		...
		return astring.charAt(0) + "*" + allStar(astring.substring(1,astring.length()));
	...
	public static String noX(String astring) {
		...
		return noX(astring.substring(0,len)) + noX(astring.substring(len));
	}	
	...
	public static int mult (int n, int m) {
		...
		if ((m < 0 && n < 0) || (m > 0 && n > 0))
			return Math.abs(n) + mult(Math.abs(n), Math.abs(m)-1);	   
		else
			return 0 - (Math.abs(n) + mult(Math.abs(n), Math.abs(m)-1));
	}
