...
public class CountInSorted {
	...
	private static int findFirst(int[] sortedA, int element, int left, int right) {
        ...
        if (sortedA[mid] == element) {
            ...
            return findFirst(sortedA, element, left, mid - 1); // Search left half
        } else {
            return findFirst(sortedA, element, mid + 1, right); // Search right half
        }
	 ...
	 private static int findLast(int[] sortedA, int element, int left, int right) {
	        ...
	        if (sortedA[mid] == element) {
	            ...
	            return findLast(sortedA, element, left, mid - 1); // Search left half
	        } else {
	            return findLast(sortedA, element, mid + 1, right); // Search right half
	        }
	    }
