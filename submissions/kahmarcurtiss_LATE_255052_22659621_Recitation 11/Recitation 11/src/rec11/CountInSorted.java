...
public class CountInSorted 
{
	...
	//
	{
		...
		if(A[mid] < target)
		{
			return help(A, target, mid + 1, high);
			//target must in right half
		}
		else if (A[mid] > target)
		{
			return help(A, target, low, mid - 1);
			//target must be in left half
		}
		else
		{
			return help(A, target, low, mid - 1) 
			+ help(A, target, mid + 1, high);
			//target found so count the target then search both sides for duplicate
		}
	}
