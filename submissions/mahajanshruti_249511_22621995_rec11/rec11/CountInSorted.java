...
public class CountInSorted {
	...
	public static int countSorted (int [] sortedA, int element) {
		...
		if(sortedA.length == 0)
			return 0;
		int first = searchforFirst(sortedA, element, 0, sortedA.length - 1);
		if(first == -1)
			return 0;
		int last = searchforLast(sortedA, element, 0, sortedA.length - 1);
		return (last - first) + 1;
	}
	
	public static int searchforFirst(int[] a, int target, int low, int high)
	{
		if(low > high)
			return -1;
		int mid = (low + high) / 2;
		...
		if(a[mid] == target) {
			if(mid == 0 || a[mid - 1] < target)
				return mid;
			return searchforFirst(a, target, low, mid - 1);
		}
		else if(a[mid] < target)
			return searchforFirst(a, target, mid+1, high);
		else
			return searchforFirst(a, target, low, mid -1);
	}
	
	public static int searchforLast(int[] a, int target, int low, int high)
	{
		if(low > high)
			return -1;
		int mid = (low + high) / 2;
		...
		if(a[mid] == target) {
			if(mid == a.length - 1 || a[mid+1] > target)
				return mid;
			return searchforLast(a, target, mid + 1, high);
		}
		else if(a[mid] < target)
			return searchforLast(a, target, mid+1, high);
		else
			return searchforLast(a, target, low, mid -1);
	}
