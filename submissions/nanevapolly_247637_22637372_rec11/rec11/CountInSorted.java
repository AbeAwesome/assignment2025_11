...
public class CountInSorted {
	...
	private static int findFirst(int[] arr, int element, int low, int high) {
	    if(low > high) {
	    	return -1;
	    }
	    int mid = (low + high) / 2;
	    comparisons++;
	    if(arr[mid] == element) {
	        int left = findFirst(arr, element, low, mid - 1);
	        return (left != -1) ? left : mid;
	    } 
	    else if(arr[mid] < element) {
	        return findFirst(arr, element, mid + 1, high);
	    } 
	    else {
	        return findFirst(arr, element, low, mid - 1);
	    }
	}
	private static int findLast(int[] arr, int element, int low, int high) {
	    if(low > high) {
	    	return -1;
	    }
	    int mid = (low + high) / 2;
	    comparisons++;
	    if(arr[mid] == element) {
	        int right = findLast(arr, element, mid + 1, high);
	        return (right != -1) ? right : mid;
	    } 
	    else if(arr[mid] < element) {
	        return findLast(arr, element, mid + 1, high);
	    } 
	    else {
	        return findLast(arr, element, low, mid - 1);
	    }
	}
