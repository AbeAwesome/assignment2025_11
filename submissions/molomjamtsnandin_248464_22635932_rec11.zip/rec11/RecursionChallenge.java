...
public class RecursionChallenge {	
	...
	public static boolean isSorted (Node node) {
		...
		return (true && isSorted(node.getNext()));
	}
	...
	public static String allStar(String astring) {
		...
		return astring.substring(0,1)+ "*" + allStar(astring.substring(1, astring.length()));
	}
	...
	public static String noX(String astring) {
		...
		if(astring.equals("x") || astring.equals("X")) {
			return "";
		}//Base case where one char is left-- checks if that char is x or not
		
		if(astring.substring(0, 1).equals("x") || astring.substring(0, 1).equals("X") ) {
			return noX(astring.substring(1,astring.length()));
		}//case where the very first char is x--if yes then gets rid of it
		
		return astring.substring(0,1) + noX(astring.substring(1, astring.length()));
	}	
