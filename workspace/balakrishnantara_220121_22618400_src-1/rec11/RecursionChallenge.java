...
public class RecursionChallenge {	
	...
	public static int sumList (Node node) {
		// using recursive list traversal
		if (node == null) {
			return 0;
		}
	...
	public static boolean isSorted (Node node) {
		// uses recursive list traversal
		if (node == null) { // empty list
			return true;
		}
		if (node.getNext() == null) { // one/last element in list
			return true;
		}
		if (node.getData() > node.getNext().getData()) { // adjacent elements are out of order
			return false;
		}
	...
	public static String noX(String astring) {
		...
		if (astring.substring(0,1).equals("x")) {
	...
	public static int mult (int n, int m) {
	    ...
	    if (m == 0 || n == 0) { // product of any number and zero is zero
	    	return 0;
	    }
	    if (n < 0 && m > 0) { // if m is negative make both integers positive by negating m 
	    	return -mult(-n, m); // divide by -1 to negate the value so the product is correct
	    }
	    else if(n < 0 && m < 0) {
	    	return mult(-n, -m);
	    }
